...
void Camera::modificaCameraAngle(int anglex, int angley){
    this->vs.obs = CalculObs(this->vs.vrp, piram.d, this->vs.angy+(angley RADIANS),this->vs.angx+(anglex RADIANS));

    this->vs.angx += (anglex RADIANS);
    this->vs.angy += (angley RADIANS);
